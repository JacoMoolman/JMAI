...
time_frame = '1h'  # Change this to '5T', '1h', '1D', etc. ME
tail_length = 30
grid_size = 10  # Number of blocks per axis
...
def create_background(ax, rs_ratio, rs_momentum, num):
    ...
    # Create white background for all cells
    ...
    for pair in rs_ratio:
        x = rs_ratio[pair].values[max(0, num-tail_length):num]
        y = rs_momentum[pair].values[max(0, num-tail_length):num]
...
def update_graph(num, rs_ratio, rs_momentum, dates, tail_length):
    ...
    create_background(ax, rs_ratio, rs_momentum, num)
    ...
    for pair in rs_ratio:
        ...
        if len(x) > 0 and len(y) > 0:
            sizes = np.linspace(100, 10, len(x))[::-1]  # Scale the dots from big to small
            plt.scatter(x, y, s=sizes, color=colors[pair])
            plt.plot(x, y, linestyle='-', linewidth=1, color=colors[pair])
    
    # Add a single date annotation for all pairs
    if num < len(dates):
        date_label = dates[num]
        plt.annotate(date_label.strftime('%Y-%m-%d %H:%M'), (0.95, 0.04), xycoords='axes fraction', ha='right')
...
ani = animation.FuncAnimation(fig, update_graph, fargs=(rs_ratio, rs_momentum, dates, tail_length), frames=len(dates), interval=1, cache_frame_data=False)
