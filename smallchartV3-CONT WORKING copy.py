...
def check_future_price(df, current_time):
...
end_date = df_5min.index[-1] - timedelta(minutes=5*FUTURE_BARS)

# Find the closest starting index
start_index = df_5min.index.searchsorted(start_date)
if start_index == len(df_5min):
    start_index -= 1

up_count, down_count, neutral_count = 0, 0, 0

for current_index in range(start_index, len(df_5min) - 5*FUTURE_BARS):
    current_time = df_5min.index[current_index]
    ...
    # Check future price movement on 1-hour timeframe
    current_hour = current_time.floor('h')
    price_direction = check_future_price(df_1hour, current_hour)
    ...
    # Plot H1 data
    h1_end_time = current_time.floor('h')
    h1_start_time = h1_end_time - timedelta(hours=NUM_BARS-1)
    h1_data = df_1hour.loc[h1_start_time:h1_end_time]
    plot_data(h1_data, 0, H1_HEIGHT, pixels)

    # Plot M5 data
    m5_end_time = current_time
    m5_start_time = m5_end_time - timedelta(minutes=5*(NUM_BARS-1))
    m5_data = df_5min.loc[m5_start_time:m5_end_time]
    plot_data(m5_data, H1_HEIGHT, M5_HEIGHT, pixels)
    ...
    if (up_count + down_count + neutral_count) % 1000 == 0:
