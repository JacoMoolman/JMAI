...
specified_start_date = '2000-07-15'  # Replace with your desired start date
...
def calculate_rs(df):
    df['Price_Relative'] = df['Close_close'] / df['Close_close'].iloc[-1]  # Relative price to the last value
...
# Normalize the RS-Ratio and RS-Momentum
for key in dfs:
    dfs[key]['RS-Ratio'] = (dfs[key]['RS-Ratio'] - dfs[key]['RS-Ratio'].min()) / (dfs[key]['RS-Ratio'].max() - dfs[key]['RS-Ratio'].min())
    dfs[key]['RS-Momentum'] = (dfs[key]['RS-Momentum'] - dfs[key]['RS-Momentum'].min()) / (dfs[key]['RS-Momentum'].max() - dfs[key]['RS-Momentum'].min())

# Extract the last 20 data points for each currency pair up to the specified start date
...
for quadrant, pos in quadrants.items():
    ax.text(0.5 + pos[0]*0.25, pos[1]*0.25, quadrant, fontsize=12, ha='center', va='center')  # Updated for normalized range [0, 1]
