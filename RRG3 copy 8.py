...
def create_background(ax, rs_ratio, rs_momentum, num):
    ...
    heatmap = heatmap / heatmap.max()  # Normalize to range 0-1
    
    for xi in range(grid_size):
        for yi in range(grid_size):
            if counts[yi, xi] > 0:
                color = heatmap[yi, xi]
                brightness = counts[yi, xi] / counts.max()
                color = np.clip(color * brightness, 0, 1)
                ax.add_patch(plt.Rectangle((xi/grid_size, yi/grid_size), 1/grid_size, 1/grid_size, color=color, alpha=brightness))
