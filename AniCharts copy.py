...
MAIN_BARS = 50  # Number of bars in the main chart
ADDITIONAL_BARS = 4  # Number of bars in the additional chart
PRICE_CHANGE_THRESHOLD = 0.005  # 0.3% change threshold (adjust as needed)
...
ax3 = fig.add_subplot(gs[0, 1])  # Additional price chart
ax4 = fig.add_subplot(gs[1, 1], sharex=ax3)  # Additional volume chart
...
def update(frame):
    ...
    # Plot additional chart
    plot_data(ax3, ax4, additional_data)
    ...
    ax2.set_xticks(main_data.index[::10])  # Show every 10th date
    ax2.set_xticklabels(main_data['Date'].dt.strftime('%Y-%m-%d %H:%M')[::10], rotation=45, ha='right')
    
    # Set x-axis limits and labels for additional chart
    ax3.set_xlim(additional_data.index.min() - 1, additional_data.index.max() + 1)
    ax4.set_xlim(additional_data.index.min() - 1, additional_data.index.max() + 1)
    ax4.set_xticks(additional_data.index)
    ax4.set_xticklabels(additional_data['Date'].dt.strftime('%Y-%m-%d %H:%M'), rotation=45, ha='right')
