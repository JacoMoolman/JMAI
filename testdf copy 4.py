...
with open(csv_file, 'r') as file:
    ...
    for line in file:
        ...
        # Create a new DataFrame with the normalized volume and other columns as floats
        normalized_df = latest_entries.copy()
        normalized_df['Volume'] = normalized_volume

        # Ensure all columns are float where applicable
        normalized_df['Open'] = normalized_df['Open'].astype(float)
        normalized_df['High'] = normalized_df['High'].astype(float)
        normalized_df['Low'] = normalized_df['Low'].astype(float)
        normalized_df['Close'] = normalized_df['Close'].astype(float)
        
        print("FIN (New DataFrame with Normalized Volume):")
        print(normalized_df)
