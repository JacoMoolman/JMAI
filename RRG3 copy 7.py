...
time_frame = 'ME'  # Change this to '5T', '1h', '1D', etc.
tail_length = 20
...
def create_background(ax, rs_ratio, rs_momentum, num):
    ...
    for xi in range(grid_size):
        for yi in range(grid_size):
            if counts[yi, xi] > 0:
                color = heatmap[yi, xi]
                ax.add_patch(plt.Rectangle((xi/grid_size, yi/grid_size), 1/grid_size, 1/grid_size, color=color, alpha=counts[yi, xi]/counts.max()))
